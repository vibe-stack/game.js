...
interface PhongMaterialPropertiesProps {
  properties: any;
  onChange: (properties: any) => void;
}

export function PhongMaterialProperties({ properties, onChange }: PhongMaterialPropertiesProps) {
  ...
  return (
    <div className="space-y-4">
      <h4 className="text-sm font-medium text-white">Phong Material Properties</h4>
      ...
      <div className="space-y-3">
        ...
        {/* Specular Color */}
        <div>
          <label className="block text-xs text-gray-400 mb-1">Specular Color</label>
          <input
            type="color"
            value={properties.specular || "#111111"}
            onChange={(e) => handlePropertyChange("specular", e.target.value)}
            className="w-full h-8 rounded border border-gray-600 bg-transparent"
          />
        </div>
        ...
        <DragInput
          label="Shininess"
          value={properties.shininess || 30}
          onChange={(value) => handlePropertyChange("shininess", value)}
          step={1}
          precision={0}
          min={0}
          max={100}
